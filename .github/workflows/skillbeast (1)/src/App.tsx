...
import { Menu, X, User as UserIcon, Download, BookOpen, BarChart, CheckCircle, Save, ArrowRight, Copy, Check, Tag, Trash2, Package, Link, ChevronDown, Mail, XCircle, Search, ChevronUp, MessageCircle, Shield, Target, Video, Infinity, Users, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function App() {
  ...
  useEffect(() => {
    if (userRole === 'admin' || user?.email === 'motrafincc@gmail.com' || user?.email === 'motrafin14@gmail.com') {
      ...
      if (activeAdminTab === 'orders' || activeAdminTab === 'overview') fetchOrders();
      if (activeAdminTab === 'coupons') fetchCoupons();
      if (activeAdminTab === 'users' || activeAdminTab === 'overview') fetchUsers();
    } else if (user) {
       ...
       <AnimatePresence>
        {isLoginOpen && (
          ...
          >
            ...
            >
              <h2 className="text-2xl font-bold mb-6">লগইন করুন</h2>
              <button onClick={handleLogin} className="w-full bg-blue-500 py-3 rounded-lg font-bold hover:bg-blue-600 transition">Google দিয়ে লগইন</button>
              <button onClick={() => setIsLoginOpen(false)} className="w-full mt-4 text-slate-400 text-sm hover:text-white transition">বন্ধ করুন</button>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
